...
def geocode(place, api_key):
    """
    STRICT geocoding:
    - Requires 'City, State, Country' (3 parts, comma-separated).
    - Accepts Geoapify result only when the returned city/town/village equals the typed city (case/diacritic-insensitive),
      and when state & country match (case-insensitive).
    ...
    if _norm(city_res) != _norm(typed_city):
        ...
        pat = r"" + re.escape(typed_city.strip()) + r""
...
def geocode_suggestions(typed, api_key, limit=7):
    """
    Return list of suggestion dicts for ambiguous city names:
      [{'label': 'City, State, Country', 'lat': ..., 'lon': ...}, ...]
    Only returns city/town/village-level matches where the city equals the typed city.
    """
    import re, json, urllib.parse, urllib.request
    out = []
    ...
    # Use first token before comma as city key
...
with row1c2:
    ...
    _typed_place = (st.session_state.get('place_input', '') or '').strip()
    _need_help = _typed_place and (_typed_place.count(",") < 2)
    _api_key = st.secrets.get("GEOAPIFY_API_KEY", "")
    if _api_key and _need_help:
        _sug = geocode_suggestions(_typed_place, _api_key, limit=7)
        if len(_sug) > 1:
            _labels = ["— Select exact place —"] + [s["label"] for s in _sug]
            _idx = st.selectbox("Similar city names found",
                                options=list(range(len(_labels))),
                                format_func=lambda i: _labels[i],
                                index=0,
                                key="pob_choice_idx")
            if _idx and 0 <= _idx < len(_labels):
                _chosen = _sug[_idx - 1]
                st.session_state['place_input'] = _chosen["label"]
                st.session_state['pob_lat'] = _chosen["lat"]
                st.session_state['pob_lon'] = _chosen["lon"]
                try:
                    st.session_state['tz_input'] = str(get_timezone_offset_simple(_chosen["lat"], _chosen["lon"]))
                    st.session_state.pop('tz_error_msg', None)
                except Exception:
                    st.session_state['tz_input'] = ''
                st.session_state['last_place_checked'] = _chosen["label"]
                st.rerun()
        elif len(_sug) == 1:
            _chosen = _sug[0]
            st.session_state['place_input'] = _chosen["label"]
    ...
    # Show resolved coordinates under the field when available
    _lat = st.session_state.get('pob_lat', None)
    _lon = st.session_state.get('pob_lon', None)
    st.text_input("Latitude", value=("" if _lat is None else f"{_lat:.6f}"), disabled=True)
    st.text_input("Longitude", value=("" if _lon is None else f"{_lon:.6f}"), disabled=True)
...
# Auto-populate UTC offset when place changes + keep lat/lon in session + normalize POB as "City, State, Country"
place_input_val = (st.session_state.get('place_input', '') or '').strip()

# Initialize lat/lon session keys if missing
if 'pob_lat' not in st.session_state: st.session_state['pob_lat'] = None
if 'pob_lon' not in st.session_state: st.session_state['pob_lon'] = None

# If the user has changed the place text since our last successful geocode, immediately clear UTC and coords
if place_input_val != st.session_state.get('last_place_checked', ''):
    st.session_state['tz_input'] = ''
    st.session_state['pob_lat'] = None
    st.session_state['pob_lon'] = None
    st.session_state.pop('tz_error_msg', None)
    st.session_state['last_place_checked'] = ''

# When place present and UTC empty (or place changed), validate & compute
if place_input_val and (place_input_val != st.session_state.get('last_place_checked', '') or not (st.session_state.get('tz_input') or '').strip()):
    try:
        ...
        if api_key:
            lat, lon, disp = geocode(place_input_val, api_key)  # strict validation (requires City, State, Country)
            # Save normalized display and coordinates
            st.session_state['place_input'] = disp
            st.session_state['pob_lat'] = lat
            st.session_state['pob_lon'] = lon

            # Auto UTC from coords (independent of DOB/TOB)
