...
def render_kundali_chalit(
    ...
    for p in placements:
        ...
        if p['shift']:
            ...
            d = (dx*dx + dy*dy) ** 0.5 or 1.0
            ux, uy = dx/d, dy/d
            # compute distance from center to rectangle edge along (ux,uy)
            eps = 1e-6
            dist_x = (mark_w/2.0) / (abs(ux) + eps)
            dist_y = (mark_h/2.0) / (abs(uy) + eps)
            edge   = min(dist_x, dist_y)
            margin = max(2.5, mark_h * 0.25)  # small gap outside the label
            sx2, sy2 = sx + ux*(edge + margin), sy + uy*(edge + margin)
            ex2 = sx2 + (ex - sx2) * shift_arrow_scale
            ey2 = sy2 + (ey - sy2) * shift_arrow_scale
            shapes.append(f'''
            <v:line style="position:absolute;z-index:5" from="{sx2},{sy2}" to="{ex2},{ey2}" strokecolor="#333333" strokeweight="1pt">
