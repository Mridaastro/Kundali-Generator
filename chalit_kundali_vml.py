...
def _apply_cusp_positioning(baseline_xy, t: float, mid_t: float, cusp_anchors,
    ...
    """Apply cusp snapping + mid-split positioning logic."""
...
def render_kundali_chalit(
    ...
    for code in ['Su','Mo','Ma','Me','Ju','Ve','Sa','Ra','Ke']:
        ...
        if h_c != h_r:
            inset_shift_pt = max(4.0, S * 0.012)  # small inset so glyph is just inside border
            ...
            if h_c == (h_r % 12) + 1:  # forward shift
                ...
                border_xy = end_anchor_h_r  # end cusp (toward next house)
                ...
                start_anchor_h_r, end_anchor_h_r = cusp_anchors[h_r]
                border_xy = start_anchor_h_r    # start cusp (from previous house)
                disp_xy   = _inset_toward_centroid(border_xy, houses[h_r], inset_shift_pt)
                shift_arrow = dict(start=disp_xy, end=chalit_xy, label=f"{degs}°")
                effective_xy = chalit_xy
    ...
    for h in range(1,13):
        cx, cy = _poly_centroid(_house_polys(S)[h])
        num_w, num_h = 10, 12
